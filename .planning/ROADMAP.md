...
**Core mental model:** A "run" is one complete prompt-to-response cycle. Its causality is encoded in `span_id`/`parent_span_id` already in the signals table — no rule inference needed. Deviation is already computed per-signal via `enrich_baseline_deviation`. The work is reconstruction, not inference.
...
- REQ-P7-02: PostgreSQL migration 011 — `session_baseline_profiles` table storing `(app_id)` → layer activation sequence, session duration percentiles, anomaly rate
- REQ-P7-03: `internal/trace/` package — `RunReconstructor`: reads `span_id`/`parent_span_id` from ClickHouse, builds typed DAG for a `trace_id`. Nodes carry signal enrichment + layer-context summary. Edges typed: parent_child, temporal. Orphaned spans (parent absent) handled via `partial.go`.
- REQ-P7-04: `internal/trace/` package — `SessionTimeline`: reads all signals for `session_id` or `conversation_id` in temporal order, groups by layer, derives session aggregates (peak baseline deviation, layer activation sequence, anomaly timeline, session duration)
- REQ-P7-05: `internal/baseline/` extension — `SessionBaselineEngine`: async 10-min compute (same cadence as signal-level baseline). Profiles keyed by `app_id`. `ComputeSessionDrift` via normalised Levenshtein on layer enum sequences. Dual-writes Redis (30m TTL) + PostgreSQL.
...
- REQ-P7-09: `GET /api/v1/alerts/{alert_id}/chain` — walks `alerts.signal_ids[]` through the span tree to reconstruct the signal chain leading to the alert. JWT auth.
- REQ-P7-10: `argus behaviour` TUI command — three views: (1) run list: recent trace_ids for an app_id with layer summary + peak deviation; (2) run detail: indented span tree coloured by deviation severity; (3) run compare: two trace_ids diffed layer-by-layer showing deviation delta. Built with `bubbletea`.
...
| REQ-P7-10 | 7 | argus behaviour TUI — run list, run detail, run compare |
